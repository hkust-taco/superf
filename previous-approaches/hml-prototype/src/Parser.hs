...
ptypeEx
  = do reserved "forall"
       quants <- many1 quantifier
       let (names,mkQuants) = unzip quants
       withIds names $ \ids ->
         do reservedOp "."
            tp <- ptype
            let qs = zipWith (\f id -> f id) mkQuants ids
            return (mkForallQ qs tp)
  <|> 
    tpfun
  <?> "type"
...
quantifier
  = do name <- identifier
       return (name, \id -> Quant id Bottom)
  <|>
    do symbol "("
       name <- identifier
       symbol ">="
    ...
    do name <- identifier
       if (isUpper (head name))
        then return (TCon name)
                ...
                return (TVar (TypeVar id Quantified))
  <?> "simple type"

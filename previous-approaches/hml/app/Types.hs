...
instance Pretty TypeVar where
   pretty (TypeVar id Quantified)  = text "@" <> pretty id
   pretty (TypeVar id Skolem)    = text "_" <> pretty id
   pretty (TypeVar id (Uni _)) = text "$" <> pretty id
...
ppAnnot (Annot qs tp)
  = let nice = niceExtend [] (quantIds qs)
    in (if null qs then empty else (text "some" <+> hsep (map (ppQuant nice) qs) <> text ". ")) <>
  ...
  = case tp of
      Forall _ _
        -> let (qs,rho) = splitQuants tp
           ...
           in text "forall" <+> hsep (map (ppQuant nice') qs) <> dot <+>
      ...
      TVar (TypeVar id (Uni ref))      -> text "$" <> ppNice nice id <> ppBound nice ref
...
ppBound nice ref
  = case unsafePerformIO (readIORef ref) of
      Equal tp             -> text "=" <> ppType nice PrecAtom tp
      Instance Bottom rank -> text "_" <> ppRank rank
      Instance tp rank     -> text "_" <> ppRank rank <> text ">=" <> parens (ppType nice PrecTop tp)
